...
# Prompt 05 — AI Agents Hub UI

## Objective

Build /dashboard/agents showing GrindCTRL can manage AI agents across business channels.

## UI source requirements

Use:
- Shadboard settings/cards for agent cards.
- Kiranism dashboard/workspace cards for overview.
- Creembase AI/admin blocks if available.
...
input types
outputs/actions
status badge: Preview-ready, Needs connection, Planned
CTA: Configure preview, Request implementation, View conversations
...
Use static config:
apps/web-next/lib/dashboard/agent-catalog.ts

## Likely files

apps/web-next/app/dashboard/agents/page.tsx
apps/web-next/components/dashboard/agent-card.tsx
apps/web-next/components/dashboard/agent-detail-preview.tsx
apps/web-next/lib/dashboard/agent-catalog.ts

## Tests

catalog has required agents
page renders cards
