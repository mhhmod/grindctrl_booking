...
# Prompt 09 — Workflows and Playground History

## Objective

Make /dashboard/workflows explain the workflow catalog and show trial preview history.

## UI source requirements

Use:
- Shadboard Kanban cards/pipeline patterns.
- Kiranism dashboard/workflow cards.
- Creembase workflow/admin blocks if available.
...
Social Inbox Routing
Implementation Request

Each card:
trigger/input
AI processing
prepared output/action
status: Active preview, Ready to connect, Planned

## Preview history

Read latest preview from localStorage handoff.
Show as latest trial preview.
If no preview, show empty state.
Do not fake database history.
...
apps/web-next/app/dashboard/workflows/page.tsx
apps/web-next/components/dashboard/workflow-catalog.tsx
apps/web-next/components/dashboard/workflow-preview-history.tsx
apps/web-next/lib/dashboard/workflow-catalog.ts
apps/web-next/lib/trial/landing-preview-handoff.ts

## Tests

catalog renders
latest preview renders if available
empty state renders if no preview
