...
describe('try-on service', () => {
  ...
  describe('generateTryOn', () => {
    ...
    describe('plan tier to provider model', () => {
      ...
      beforeEach(() => {
        TIER_ENV_NAMES.forEach((name) => delete process.env[name]);
        process.env.TRYON_MODE = 'live';
        beginTryOnJobMock.mockImplementation((input: { jobId: string; modelKey: string }) =>
          Promise.resolve({
            jobId: input.jobId,
            created: true,
            status: 'queued',
        ...
        runImageGenerationMock.mockResolvedValue({
          ...
          createdAt: '2026-08-31T00:00:00.000Z',
          meta: { runtime: 'live', provider: 'provider', costEstimate: 0.01 },
        });
      });
      ...
      async function generateFor(tier: string | null) {
        getShopEntitlementMock.mockResolvedValue({ modelKey: tier });
        const auth = storefrontAuthorization();
        await generateTryOn(auth, 'upload', 'data:image/png;base64,AAAA');
        return {
          auth,
          providerModel: runImageGenerationMock.mock.calls[0][4],
          reservedTier: beginTryOnJobMock.mock.calls[0][0].modelKey,
        };
      }
      ...
      it('names the model that was actually called when the provider fails', async () => {
        ...
        await expect(
          generateTryOn(storefrontAuthorization(), 'upload', 'data:image/png;base64,AAAA'),
