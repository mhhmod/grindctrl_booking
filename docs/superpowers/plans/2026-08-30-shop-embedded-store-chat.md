...
All 12 tasks shipped on `feat/shop-embedded-store-chat`, implemented mostly by Codex CLI (`codex-delegate`, one task per dispatch, `danger-full-access` sandbox — this machine's Codex `workspace-write` sandbox cannot spawn subprocesses at all, a Codex/Windows bug unrelated to this project) with every diff independently re-verified (tests re-run, diff read line-by-line, `tsc`/build/lint re-run) before commit, not taken on the implementer's self-report. Final state: **774/774 tests passing** (152 files, up from Phase 1's 729), `tsc --noEmit` clean, `next lint` unchanged at the same 4 pre-existing `components/assistant/*` errors, production build succeeds, and a manual browser smoke test confirmed the embedded shell (header, tab switching, dark mode, both tabs' correct out-of-iframe error states).

**Scope, confirmed as planned:** five editors (Appearance, Behaviour, AI & Knowledge, Support desk, Installation) got the `actions` prop; `conversations-panel.tsx` is untouched (`git diff main` on it is empty). Five routes shipped — `draft`, `publish`, `enable`, `state`, `knowledge` — not the spec's illustrative `draft`/`publish`/`state`/`knowledge`/`thread` list: `enable` replaced `thread`, because `install-card.tsx`'s `setMessengerEnabled` needed a route the original spec didn't name, and `thread` was correctly deferred with Conversations per the Scope note above. No route under `app/api/shopify/store-chat/*` imports `requireOwnedSite` or `@clerk/nextjs/server` — confirmed by grep.

**Defects found in this plan's own code during execution, fixed at the source (not silently patched around):**

1. **Task 4's test had a fake-timer ordering bug.** `.rejects.toThrow(...)` was attached to `pending` *after* `vi.advanceTimersByTimeAsync(6000)` had already settled the promise, so vitest reported an unhandled rejection instead of a passing assertion — the timer fired before anything was listening. Fixed by attaching the `.rejects` expectation before advancing timers (`const pending = expect(getShopifySessionToken()).rejects.toThrow(...); await vi.advanceTimersByTimeAsync(6000); await pending;`). Caught by Codex refusing to guess past a failing gate; fixed directly in both the plan and the shipped test.

2. **Task 9's route swallowed no errors — it leaked all of them.** The plan's original catch-all (`const message = error instanceof Error ? error.message : 'Action failed...'; return NextResponse.json({ ok: false, error: message }, ...)`) surfaced *any* thrown error verbatim to an untrusted client, unlike `actions.ts`'s `addKnowledge`/`syncKnowledge`, which only expose messages matching a safe fetch-failure pattern (`/https|URL|page|readable/i`) and genericize everything else. A raw Postgres/Supabase error (e.g. a constraint-violation message naming a column) would have reached the embedded UI. Found during my own pre-dispatch review of the plan text, fixed before Codex ever implemented it — added the same filter plus a new test (`'genericizes an error that does not match the friendly-message pattern'`) proving the genericization actually fires, not just that the safe case still works.

3. **Two files were missing an explicit `React` import.** This codebase's vitest/vite JSX transform requires `import React from 'react'` in every file returning JSX — confirmed by the fact that every existing component file in `components/dashboard/messenger/` already does this, and by the concrete `ReferenceError: React is not defined` vitest threw when `store-chat-embedded.tsx`'s first draft omitted it. Codex caught and fixed that one itself (the plan's own code sample for the file was missing the import). `app-shell.tsx` had the identical gap but no test to catch it — found in my own diff review after Codex's dispatch reported all-green, since a missing React import there is silent until something renders it; fixed the same way (`import React, { useState } from 'react';`) for consistency with the rest of the codebase, even though Next's own SWC build would likely have tolerated the omission.
